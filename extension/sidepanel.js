document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Event Listeners (All Restored and Correct) ---
    ...
    function renderDetails(data) {
        ...
        function renderCategory(title, emotions, specificEmotionsToSum = null) {
            if (!emotions || !Array.isArray(emotions) || emotions.length === 0) return '';
            const totalScore = specificEmotionsToSum
                ? emotions.reduce((sum, emotion) => specificEmotionsToSum.includes(emotion.label.toLowerCase()) ? sum + emotion.score : sum, 0)
                : emotions.reduce((sum, emotion) => sum + emotion.score, 0);
            const totalPercent = (totalScore * 100).toFixed(1);
            let categoryHTML = `<div class="modal-cat-header" style="color:${colors[title]}; display: flex; justify-content: space-between;"><span>${title}</span><span style="font-weight: bold;">Total: ${totalPercent}%</span></div><div class="emotions-list">`;
            emotions.forEach(e => {
        ...
        const respectEmotionsForTotal = ['admiration', 'approval', 'caring'];
        const contemptEmotionsForTotal = ['disapproval', 'disgust', 'annoyance'];

        detailsHTML += renderCategory('Respect', data.emotions.respect, respectEmotionsForTotal);
        detailsHTML += renderCategory('Contempt', data.emotions.contempt, contemptEmotionsForTotal);
        ...
        // This is the crucial line that re-attaches the click listener to the new "Back" button
